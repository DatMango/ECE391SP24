...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        bootblock = (boot_block_t*) mod->mod_start; // pointing to bootblock address 
    ...
    idt_initializer();
    i8259_init();
    init_rtc(2);      //initialize rtc before keyboard if we want to test rtc
    init_keyboard();  //initialize keyboard beofre rtc if we want to test keyboard
    init_pit();
